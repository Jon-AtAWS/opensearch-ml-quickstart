...
"""
Test runner for client module tests.

This script runs the unit tests for the client directory.
...
def run_unit_tests():
    ...
    try:
        ...
        from tests.unit.client.test_helper import TestGetClient, TestParseVersion, TestCheckClientVersion
        from tests.unit.client.test_index_utils import (
            TestHandleIndexCreation, TestHandleDataLoading, 
            TestLoadCategory, TestSendBulkIgnoreExceptions, TestGetIndexSize
        )
        from tests.unit.client.test_os_ml_client_wrapper import TestOsMlClientWrapper
        
        # Run tests manually (basic test runner)
        test_classes = [
            TestGetClient, TestParseVersion, TestCheckClientVersion,
            TestHandleIndexCreation, TestHandleDataLoading, 
            TestLoadCategory, TestSendBulkIgnoreExceptions, TestGetIndexSize,
            TestOsMlClientWrapper
        ]
...
def run_integration_tests():
    ...
    try:
        from tests.unit.client.test_integration import test_os_client, test_aos_client
        
        print("\nTesting OS client connection...")
        try:
            test_os_client()
            print("  ✓ OS client test passed")
        except Exception as e:
            print(f"  ✗ OS client test failed: {e}")
        
        print("\nTesting AOS client connection...")
        try:
            test_aos_client()
            print("  ✓ AOS client test passed")
        except Exception as e:
            print(f"  ✗ AOS client test failed: {e}")
