...
def create_workflow_template(index_name, pipeline_name, model_id, model_dimension, base_mapping_path):
    """
    ...
    - Proper mappings and settings from BASE_MAPPING_PATH
    ...
    index_config = {
        ...
        "model_dimensions": model_dimension
    }
...
def verify_index_creation(client, index_name):
    ...
    try:
        ...
        if "chunk_embedding" in mappings.get("properties", {}):
            ...
            logging.warning("Vector field 'chunk_embedding' not found in index mappings")
            return False
...
def setup_and_run_workflow(client, workflow_template):
    ...
    try:
        ...
        provision_response = client.os_client.transport.perform_request(
            "POST",
            f"/_plugins/_flow_framework/workflow/{workflow_id}/_provision"
        )
...
def interactive_search_loop(client, index_name, ml_model):
    ...
    while True:
        try:
            ...
            # Build neural search query for dense vector search
            search_query = {
                "size": 5,
                "query": {
                    "neural": {
            ...
            # Print search results using the new function
...
def main():
    ...
    if index_exists and not args.delete_existing_index:
        ...
        template_success, workflow_id = setup_and_run_workflow(client, workflow_template)
    
    if template_success:
        logging.info(f"Successfully set up and executed workflow template: {workflow_id}")
        
        # Verify that the index was created successfully
        if verify_index_creation(client, index_name):
            logging.info("Index and pipeline are ready for data loading")
        else:
            logging.warning("Index verification failed, but continuing with data loading")
    else:
