...
"""Multi-CPR semantic search using OpenSearch workflows for automated setup.

This example demonstrates semantic search over the Multi-CPR Chinese dataset
using OpenSearch workflows to automatically deploy Bedrock embedding models
and configure the search pipeline.

Usage:
    # Search e-commerce domain with 500 passages
    python examples/multi_cpr_semantic_search_workflow.py -o aos -c ecom -n 500

    # Search all domains with interactive mode
    python examples/multi_cpr_semantic_search_workflow.py -o aos -c ecom video medical -n 1000

    # Single query mode
    python examples/multi_cpr_semantic_search_workflow.py -o aos -c medical -n 200 -q "感冒吃什么药"
"""
...
def fix_connector_region(client, connector_id, model_id, target_region):
    ...
    try:
        ...
        if target_region not in current_url:
            new_url = f"https://bedrock-runtime.{target_region}.amazonaws.com/model/amazon.titan-embed-text-v2:0/invoke"
...
def main():
    ...
    host_type = "aos"
    model_host = "bedrock"
    index_name = "multi_cpr_semantic_workflow"
    embedding_type = "dense"
    pipeline_name = "multi-cpr-semantic-workflow-pipeline"
    ...
    if args.no_load:
        ...
        logging.info("Setting up workflow for Multi-CPR semantic search...")
        ...
        body = {
            ...
            "create_ingest_pipeline.description": "Multi-CPR text embedding pipeline",
        ...
        # Use Multi-CPR specific mapping instead of base mapping
        mapping = dataset.get_index_mapping()
        try:
            client.os_client.indices.put_mapping(index=index_name, body={"properties": mapping["properties"]})
            logging.info(f"Multi-CPR mapping updated for index {index_name}")
        except Exception as e:
    ...
    logging.info("Setup complete! Starting interactive Multi-CPR search interface...")
