...
class AiConnectorHelper:
    ...
    def search_model_group(self, model_group_name):
        payload = {"query": {"term": {"name.keyword": {"value": model_group_name}}}}
        headers = {"Content-Type": "application/json"}
        r = requests.post(
    ...
    def create_model_group(self, model_group_name, description):
        ...
        if (
            "no such index [.plugins-ml-model-group]"
            not in str(search_model_group_response)
            and search_model_group_response["hits"]["total"]["value"] > 0
        ):
            return search_model_group_response["hits"]["hits"][0]["_id"]
        payload = {"name": model_group_name, "description": description}
        headers = {"Content-Type": "application/json"}
        r = requests.post(
